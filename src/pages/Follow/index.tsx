...
import './index.scss'
import { followAnchorListReq } from '@/api/common'
...
function FollowPage() {
  ...
  const initData = async () => {
    ...
    setLoading(true)
    const res = await followAnchorListReq()
  ...
  return <div className='follow-page'>
    <div className='title'>
      <FormattedMessage id='scdzb' /><span className='count'>({list.length})</span>
      {
        !loading && !list.length ? <Empty description={<FormattedMessage id='nofollow' />} /> : <div className='list'>
        {
